...
public class MainUser {
    ...
    public final Client.ResultHandler defaultHandler = new MainUser.DefaultHandler();
...
//    private final String commandsLine = "Enter command (gcs - GetChats, gc <chatId> - GetChat, me - GetMe, sm <chatId> <message> - SendMessage, lo - LogOut, q - Quit): ";
    ...
    public void start() throws InterruptedException {
        ...
        new Thread(() -> {
            while (!needQuit && haveAuthorization) {
                getCommand();
            }
        }).start();
    }
    ...
    private class UpdateHandler implements Client.ResultHandler {
        ...
        private final MainUser clientMainUser;

        public UpdateHandler(MainUser clientMainUser) {
            this.clientMainUser = clientMainUser;
        }
        @Override
        public void onResult(TdApi.Object object) {
            switch (object.getConstructor()) {
                ...
                case TdApi.UpdateNewChat.CONSTRUCTOR: {
                    ...
                    if (chat.title.equals(clientMainUser.receiveGroupName)) {
                        clientMainUser.receiveGroupId = chat.id;
                    }
                    synchronized (chat) {
                ...
                case TdApi.UpdateChatLastMessage.CONSTRUCTOR: {
                    ...
                    if (updateChat.lastMessage.id <= clientMainUser.lastMsgId.getOrDefault(updateChat.chatId, 0L)) {
                        break;
                    }
                    lastMsgId.put(updateChat.chatId, updateChat.lastMessage.id);
                    for (LastMsgHandlerType handlerType : LastMsgHandlerType.values()) {
                        handlerType.getHandler().handle(clientMainUser, updateChat);
                    }
                    break;
                }
    ...
    private void onAuthorizationStateUpdated(TdApi.AuthorizationState authorizationState) {
        ...
        switch (this.authorizationState.getConstructor()) {
            ...
            case TdApi.AuthorizationStateClosing.CONSTRUCTOR:
                haveAuthorization = false;
                print("main user Closing");
                break;
            case TdApi.AuthorizationStateClosed.CONSTRUCTOR:
                print("main user Closed");
                if (!needQuit) {
